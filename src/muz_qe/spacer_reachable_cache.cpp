...
namespace spacer {
    ...
    void reachable_cache::add_reachable(expr * cube) {
        ...
        switch (m_cache_mode) {
        case datalog::SPACER_NO_CACHE: 
            break;
            
        case datalog::SPACER_HASH_CACHE: 
            m_stats.m_inserts++;
            m_cache.insert(cube);
            m_ref_holder.push_back(cube);
            break;
            
        case datalog::SPACER_CONSTRAINT_CACHE: 
            m_stats.m_inserts++;
            TRACE("spacer", tout << mk_pp(cube, m) << "\n";);
    ...
    bool reachable_cache::is_reachable(expr * cube) {
        ...
        case datalog::SPACER_CONSTRAINT_CACHE: {        
            ...
            TRACE("spacer", tout << "is_reachable: " << res << " " << mk_pp(cube, m) << "\n";);
