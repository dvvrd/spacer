...
namespace datalog {
    ...
    start:
        ...
        if (r->get_tail_size()==0 && m_context.get_rule_manager().is_fact(chead)) {
            ...
            rule_ref new_rule(m_context.get_rule_manager().mk(r, chead, r->get_names()), m_context.get_rule_manager());
        ...
        rule_ref& res)
    {
        ...
        res = m_context.get_rule_manager().mk( r->get_head(), tails.size(), tails.c_ptr(), r->get_names(), tails_negated.c_ptr());
