...
namespace datalog {
    ...
    rule_set * mk_magic_symbolic::operator()(rule_set const & source) {
        ...
        for (unsigned i = 0; i < sz; ++i) {            
            ...
            new_rule = rm.mk(mk_ans(r.get_head()), tail.size(), tail.c_ptr(), r.get_names(), neg.c_ptr(), true);
            result->add_rule(new_rule);                
            if (source.is_output_predicate(r.get_decl())) {
                result->set_output_predicate(new_rule->get_decl());
                new_rule = rm.mk(mk_query(r.get_head()), 0, 0, r.get_names(), 0, true);
                result->add_rule(new_rule);
            }

            for (unsigned j = 0; j < utsz; ++j) {
                new_rule = rm.mk(mk_query(r.get_tail(j)), tail.size()-utsz+j, tail.c_ptr(), r.get_names(), neg.c_ptr(), true);
                result->add_rule(new_rule);
            }            
