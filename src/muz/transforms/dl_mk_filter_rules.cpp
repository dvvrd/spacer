    ...
    */
    func_decl *  mk_filter_rules::mk_filter_decl(app * pred, var_idx_set const & non_local_vars) {
        ...
        if (!m_tail2filter.find(key, filter_decl)) {
            ...
            rule * filter_rule = m_context.get_rule_manager().mk(filter_head, 1, &filter_tail, svector<symbol>(), (const bool *)0);
    ...
    void mk_filter_rules::process(rule * r) {
        ...
        if (rule_modified) {
            ...
            rule * new_rule = m_context.get_rule_manager().mk(new_head, new_tail.size(), new_tail.c_ptr(), r->get_names(), new_is_negated.c_ptr());
