...
namespace datalog {
    ...
    void mk_interp_tail_simplifier::rule_substitution::get_result(rule_ref & res) {
        ...
        res = m_context.get_rule_manager().mk(m_head, m_tail.size(), m_tail.c_ptr(), m_rule->name(), m_neg.c_ptr());
...
#define IS_FLEX(_x) (is_var(_x) || m.is_value(_x))
    ...
    start:
        ...
        if (modified) {
            ...
            res = m_context.get_rule_manager().mk(head, m_tail.size(), m_tail.c_ptr(), r->name(), m_tail_neg.c_ptr());
