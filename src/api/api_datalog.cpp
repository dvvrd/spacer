...
extern "C" {
    ...
    Z3_lbool Z3_API Z3_fixedpoint_prepare_query(Z3_context c,Z3_fixedpoint d, Z3_ast q) {
        Z3_TRY;
        //no logging
        RESET_ERROR_CODE();
        lbool r = l_undef;
    ...
    Z3_lbool Z3_API Z3_fixedpoint_process_result(Z3_context c,Z3_fixedpoint d) {
        Z3_TRY;
        //no logging
        RESET_ERROR_CODE();
        lbool r = l_undef;
        ...
        api::context::set_interruptable si(*(mk_c(c)), eh);        
        {
            scoped_timer timer(timeout, &eh);
            try {
                r = to_fixedpoint_ref(d)->ctx().process_result();
            }
            catch (z3_exception& ex) {
                mk_c(c)->handle_exception(ex);
                r = l_undef;
